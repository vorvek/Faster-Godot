...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the Faster-Godot repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (faster-godot.tar.gz)"
    echo "  -v Faster-Godot version for file name (e.g. 4.6.3b)"
...
if [ ! -z "$script_friendly_name" ]; then
  NAME=faster-godot-source
else
  if [ ! -z "$godot_version" ]; then
    NAME=faster-godot-$godot_version-source
  else
    NAME=faster-godot-$HEAD-source
  fi
fi

CURDIR=$(pwd)
TMPDIR=$(mktemp -d -t faster-godot-XXXXXX)
