...
void main() {
	...
	if (total_density > 0.00005) {
		...
		{ //area lights
			...
			// ElementType order in the fork: OMNI(0), SPOT(1), DECAL(2), REFLECTION_PROBE(3), AREA(4),
			// so area uses cluster_type_size * 4 (NOT upstream's * 2). Matches scene_forward_clustered.glsl.
			uint cluster_area_offset = cluster_offset + params.cluster_type_size * 4;
