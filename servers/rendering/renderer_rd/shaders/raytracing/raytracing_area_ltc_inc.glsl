// RT port of the rasterizer's linearly-transformed-cosine area-light stack (area_lights_inc.glsl).
// The texture-free integration math is verbatim; the atlas fetches take the bindless atlas index
// instead of a (texture2D, sampler) pair so the nonuniform indexing stays at the texel access, and
// the specular path takes the set-0 ltc_lut1/ltc_lut2 samplers. Requires bindless_textures, the
// sampler constants, and ltc_lut1/ltc_lut2 to be in scope (they are, from the stage includes).
...
float rt_ltc_acos_approx(float p_x) {
	float x = abs(p_x);
	float res = -0.156583 * x + (M_PI / 2.0);
	res *= sqrt(1.0 - x);
...
// Adapted from area_lights_inc.glsl fetch_ltc_lod: the (texture2D, sampler) pair is replaced by the
// bindless atlas index so the nonuniform indexing stays at the texel access.
vec3 rt_fetch_ltc_lod(vec2 uv, vec4 texture_rect, float lod, float max_mipmap, uint atlas_idx) {
	...
	vec4 sample_col_low = textureLod(sampler2D(bindless_textures[nonuniformEXT(atlas_idx)], SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), sample_pos, low);
	vec4 sample_col_high = textureLod(sampler2D(bindless_textures[nonuniformEXT(atlas_idx)], SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), sample_pos, high);
...
// Adapted from area_lights_inc.glsl fetch_ltc_filtered_texture_with_form_factor: the form-factor
// math is verbatim; the final fetch takes the bindless atlas index. The Item 4 adaptive mip is the
// solid-angle-derived lod computed below.
vec3 rt_fetch_ltc_filtered_texture_with_form_factor(vec4 texture_rect, vec3 L[4], float max_mipmap, uint atlas_idx) {
	...
	return rt_fetch_ltc_lod(vec2(1.0) - uv, texture_rect, lod, max_mipmap, atlas_idx);
}

// Adapted from area_lights_inc.glsl ltc_evaluate: integration math verbatim; the textured branch
// takes the bindless atlas index, and acos_approx is rt_ltc_acos_approx.
void rt_ltc_evaluate(vec3 normal, vec3 eye_vec, mat3 M_inv, vec3 points[4], vec4 texture_rect, float max_mipmap, uint atlas_idx, out float integral, out vec3 tex_color) {
	// default is white
	tex_color = vec3(1.0);
	...
	if (texture_rect != vec4(0.0)) {
		tex_color = vec3(rt_fetch_ltc_filtered_texture_with_form_factor(texture_rect, L_unclipped, max_mipmap, atlas_idx));
	}
...
// Adapted from area_lights_inc.glsl ltc_evaluate_specular: the LUT samplers are the global set-0
// ltc_lut1/ltc_lut2 (dropped from the signature); acos_approx is rt_ltc_acos_approx; the inner
// evaluate is rt_ltc_evaluate.
void rt_ltc_evaluate_specular(vec3 normal, vec3 eye_vec, float roughness, vec3 points[4], vec4 texture_rect, float max_mipmap, uint atlas_idx, out float ltc_specular, out vec2 fresnel, out vec3 ltc_specular_tex_color) {
	float theta = rt_ltc_acos_approx(dot(normal, eye_vec));
	...
	rt_ltc_evaluate(normal, eye_vec, M_inv, points, texture_rect, max_mipmap, atlas_idx, ltc_specular, ltc_specular_tex_color);
