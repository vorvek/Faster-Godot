...
pub fn is_class_method_deleted(class_name: &TyName, method: &JsonClassMethod, ctx: &mut Context) -> bool {
    ...
    match (class_name.godot_ty.as_str(), method.name.as_str()) {
        ...
        // Faster-Godot omits raytracing-only rendering API types from the generated extension API.
        | ("Environment", "set_pathtracing_denoiser")
        | ("Environment", "get_pathtracing_denoiser")
        | ("RenderingDevice", "raytracing_pipeline_create")
        | ("RenderingDevice", "blas_create")
        | ("RenderingDevice", "tlas_build")
        => true,
